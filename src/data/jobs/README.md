...
| `location`        | string — `City, Country` segments; multiple locations separated by a pipe (`City, Country \| City, Country`); region-only values (`Europe`, `Worldwide`) allowed | no       | card (verbatim), country facet — full contract + cleanup requests in [docs/jobs-location-parsing.md](../../../docs/jobs-location-parsing.md) |
