...
| `SeoJsonLd.astro` via head slot (P5) | **Kept WebSite/WebPage JSON-LD inside `Layout`** (gated to non-articles) | Same output, zero per-page churn, richer than v6 (no hub-page JSON-LD upstream). See *Why JSON-LD stayed inline* below for the full reasoning — extraction is now active backlog work (low-risk, readability-motivated, not a merge-cost win). → backlog #2 |
...
| Adopt v6 CI as a PR gate (P7) | **Added `ci.yml` as `workflow_dispatch` (manual-only)** | Maintainer's call: Vercel already builds deploys; avoid a redundant auto-gate. Reassessed 2026-07-03 — see backlog #4. → backlog #4 |
| Migrate `pnpm-workspace` → `allowBuilds` (P7) | **Kept `onlyBuiltDependencies`** | `allowBuilds` is pnpm-11 syntax; we were on pnpm 10. pnpm 11 has since shipped (11.9.0) and our Node floor (22.12+) already satisfies its Node 22+ requirement, so the original "when/if we move to pnpm 11" gate is no longer a real blocker, just undone work. → backlog #5 |

Everything still open lives in **[2026-07-03-astropaper-v6-backlog.md](./2026-07-03-astropaper-v6-backlog.md)** — genuinely pending work only. **Component alignment** (Header/Footer/Breadcrumb/Tag/LinkButton staying custom) remains a settled divergence recorded in *Per-subsystem decisions* below, not tracked as backlog. `SeoJsonLd` extraction and the `withBase`/locale layer were reassessed 2026-07-03 (see below) and **are** now active backlog items — they were settled-as-"no plan to revisit" only through 2026-07-03. None of it blocked the merge, and none of it is production-blocking now. **P8 (preview cutover)** — the Sveltia `/admin` round-trip at the new paths + Vercel env scope — was validated via PR #16 into `draft`; the site has since shipped to `main`/production (PR #17) with no route regressions observed.
