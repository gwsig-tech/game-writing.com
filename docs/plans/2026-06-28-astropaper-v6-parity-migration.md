...
| `SeoJsonLd.astro` via head slot (P5) | **Kept WebSite/WebPage JSON-LD inside `Layout`** (gated to non-articles) | Same output, zero per-page churn, richer than v6 (no hub-page JSON-LD upstream). Settled — see *Per-subsystem decisions* below, not tracked as backlog. |
...
| Adopt v6 CI as a PR gate (P7) | **Added `ci.yml` as `workflow_dispatch` (manual-only)** | Maintainer's call: Vercel already builds deploys; avoid a redundant auto-gate. → backlog #2 |
| Migrate `pnpm-workspace` → `allowBuilds` (P7) | **Kept `onlyBuiltDependencies`** | `allowBuilds` is pnpm-11 syntax; we're on pnpm 10. → backlog #2 |

Everything still open lives in **[2026-07-03-astropaper-v6-backlog.md](./2026-07-03-astropaper-v6-backlog.md)** — genuinely pending work only. Settled divergences that won't be revisited (component alignment, `SeoJsonLd` extraction, the `withBase` layer) stay recorded in *Per-subsystem decisions* below, not in the backlog. None of it blocked the merge, and none of it is production-blocking now. **P8 (preview cutover)** — the Sveltia `/admin` round-trip at the new paths + Vercel env scope — was validated via PR #16 into `draft`; the site has since shipped to `main`/production (PR #17) with no route regressions observed.
