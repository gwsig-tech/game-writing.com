...
**Supersedes** [`2026-06-26-astropaper-v6-adoption.md`](./2026-06-26-astropaper-v6-adoption.md) (written before the recent conformance + meta work, now stale). **Status: ✅ SHIPPED.** P0–P7 complete; merged to `draft` via PR #16 (`c560114`) and propagated to `main`/production via PR #17 (`5049d18`). A first follow-up polish round (getPath split, v6 `theme.ts`, image lightbox, OG generator modernization) shipped 2026-07-03, and a **second maintenance round later that same day** closed most of what the first round left open: `SeoJsonLd` extraction (`8f9c60c`), Prettier reconciled to upstream's `printWidth: 80` + a one-time repo-wide format pass + LF line-ending normalization (`ef0af53`/`f0eb1d7`), Tailwind `@source not "../../docs"` (stops docs prose polluting the CSS bundle, `62e745b`), CI activated as a PR gate trimmed to lint+format (`640072a`), pnpm 11 with a `packageManager` pin (`2872399`), and the Decision-B title-suffix centralization (`e970dbc`). The `jm-astropaper-v6` **worktree and branch were merged into `draft` and removed** in that round — the repo is a single working tree again. See **[Execution outcome & deviations](#execution-outcome--deviations)** below for what actually shipped vs planned, and the **[backlog](./2026-07-03-astropaper-v6-backlog.md)** for the little still open (none of it production-blocking). The phased roadmap further down is the original plan, kept as the historical record.
...
| Split `getPath` → `getPostSlug`/`getPostUrl` (P3/P6) | Deferred at first (kept `getPath`); **✅ adopted 2026-07-03** — replaced with `getPostPaths.ts` (`getPostSlug`/`getPostUrl`), 5 call sites updated | Not needed for URL parity at the time (identical `/posts/<slug>`); the split's payoff (locale-aware URLs) was picked up in the first backlog polish round. |
| `SeoJsonLd.astro` via head slot (P5) | Kept inline at first; **✅ extracted 2026-07-03** (`8f9c60c`) — pure refactor, byte-identical JSON-LD output | See *Why JSON-LD stayed inline* below for why it was kept inline originally and why extraction was worth doing anyway (readability, not merge-cost). |
| Adopt v6 `theme.ts` + `.dark` (P4/P6) | Deferred at first (kept our `theme.ts`); **✅ adopted 2026-07-03** — `window.__theme`, additive `.dark` class, no external `window.theme` API | The "dual theme button" concern turned out moot: `Header.astro`'s own script already forwards the mobile button's click to the desktop button, independent of `theme.ts`'s internals — no Header changes were needed. |
| Conform shared components to v6 (P6) | **Kept custom** Header/Footer/Breadcrumb/Tag/LinkButton | Heavily diverged (SIG nav/footer); v6 verbatim would erase customizations. Did adopt Datetime (muted dates), glob Socials, ResponsiveTable, and the colocated post `_components`. Settled — see *Per-subsystem decisions* below, not tracked as backlog. |
| OG generator resvg→sharp + self-hosted fonts (P5/P7, Decision 3a) | Deferred at first (left dormant resvg generator); **✅ modernized 2026-07-03** — kept our per-post satori templates (upstream's own v6.1.0 replacement turned out to be a single generic site-wide image, not per-post, so adopting it verbatim would have been a capability downgrade), swapped `@resvg/resvg-js`→`sharp`, self-hosted the font via `@fontsource/ibm-plex-mono` | `dynamicOgImage:false` → it never runs in production either way; zero functional impact, but the build-time Google-Fonts fetch fragility is now gone too. |
| Adopt `resolveDefaultOgImagePath` (P5) | **Kept inline OG default** | Avoids pulling in the deferred `withBase`. |
| Image lightbox (P6) | Deferred at first; **✅ ported 2026-07-03** — v6's click-to-zoom script (pinch-zoom, focus trap, Escape/outside-click close) ported into the post route | Was non-essential at merge time; picked up in the first backlog polish round. |
| Pull-update 10 examples + pull-new `astro-paper-6` (P7) | Pulled `astro-paper-6` only at first; **✅ all 10 examples refreshed 2026-07-03** (`7982633`) — v6.1.0 bodies verbatim (3 became MDX w/ `ResponsiveTable`; `predefined-color-schemes` → `_color-schemes/` + 7 colocated PNGs, mirroring upstream) | Kept `draft: true` + our Prettier formatting; only real asset need was the scheme PNGs (`forrest-gump-quote.png` already present; the "13 assets / 4 path conventions" were mostly in-fence documentation, not rendered refs). Routes/sitemap verified unchanged. |
| Adopt v6 CI as a PR gate (P7) | Manual-only at first; **✅ activated 2026-07-03 in trimmed form** (`640072a`) — `pull_request` gate running only lint + `format:check`; the redundant build step was dropped (Vercel already builds/typechecks every PR) | Unblocked by the same-day formatting cleanup (printWidth 80 + format pass — `format:check` now green repo-wide). Making the check *required* (branch protection, GitHub UI) is backlog #8, post-push. |
| Migrate `pnpm-workspace` → `allowBuilds` (P7) | Kept `onlyBuiltDependencies` at first (pnpm-11 syntax; we were on 10); **✅ migrated 2026-07-03** (`2872399`) — pnpm 11.9.0, `allowBuilds` map, and a `package.json#packageManager` pin as the single version source for local (auto-switch), CI (`pnpm/action-setup` reads it), and Vercel (corepack) | Verified: fresh install clean, lockfile byte-identical, full build green. pnpm 11's `minimumReleaseAge` default kept (noted in the workspace file). |

Everything still open lives in **[2026-07-03-astropaper-v6-backlog.md](./2026-07-03-astropaper-v6-backlog.md)** — genuinely pending work only. **Component alignment** (Header/Footer/Breadcrumb/Tag/LinkButton staying custom) remains a settled divergence recorded in *Per-subsystem decisions* below, not tracked as backlog. `SeoJsonLd` extraction and the `withBase`/locale layer were reassessed 2026-07-03 (see below); the extraction was reopened **and shipped the same day** (`8f9c60c`), while the locale layer remains the backlog's #3. None of it blocked the merge, and none of it is production-blocking now. **P8 (preview cutover)** — the Sveltia `/admin` round-trip at the new paths + Vercel env scope — was validated via PR #16 into `draft`; the site has since shipped to `main`/production (PR #17) with no route regressions observed.

## Follow-up decisions (2026-07-03)

Two items originally recorded above as settled divergences were reopened this date after a closer look at what upstream actually does today and at the SIG's own long-term needs. Both are now tracked as active backlog items (numbers refer to [2026-07-03-astropaper-v6-backlog.md](./2026-07-03-astropaper-v6-backlog.md)); this section records *why* they were reopened and what's settled vs. still open about each.

### Why JSON-LD stayed inline (and why extraction is now worth doing anyway)

First, a correction to the original framing: **upstream does not have a `SeoJsonLd.astro`.** Checked against `upstream/main` (v6.1.0 @ `4c33a60`) directly — there is no such file anywhere in the tree, and `Layout.astro` upstream emits **zero** structured data for non-article pages (only `PostLayout.astro` emits `BlogPosting`, confirmed by grepping every `.astro` file for `ld+json`/`schema.org`). "SeoJsonLd.astro" was **our own proposed name** for what a v6-style extraction would look like — we were never diverging from an upstream pattern by keeping it inline, because upstream has no pattern here at all.

Our implementation ([Layout.astro](../../src/layouts/Layout.astro), ~15 lines) computes a `WebSite`/`WebPage` object gated on `pubDatetime` (so article pages don't double-emit against `PostLayout`'s `BlogPosting`) and inlines it as a `<script type="application/ld+json">`.

**Does extracting it reduce future upstream-merge cost?** No — and this is the reason it was originally left inline rather than a mere style preference. `Layout.astro` is already one of our most heavily customized files (Vercel analytics, our favicon/PWA set, the theme FOUC script — none of which exist upstream), so every future upstream `Layout.astro` pull requires manual re-diffing regardless of whether the JSON-LD logic lives inline or in a separate component. Extracting to `SeoJsonLd.astro` would still require a call site (`<SeoJsonLd {...props} />`) inserted into `Layout.astro` that isn't in upstream — the file-level diff against upstream is the same size either way. So extraction buys nothing on the merge-cost axis, because there is no upstream analog to converge toward.

**So why do it anyway?** Purely for **readability**, now that the block has been sitting inline for a while — and because it can be done as a pure refactor (identical JSON-LD output, `pnpm build` diffed byte-for-byte before/after) with no behavior risk. That's a different justification than "match upstream," but a legitimate one on its own, and low-risk enough to schedule rather than leave as a someday-maybe.

**Outcome:** shipped 2026-07-03 (`8f9c60c`) exactly as scoped — `src/components/SeoJsonLd.astro` with `Layout` keeping the `!pubDatetime` gate; typecheck clean, output identical.

### Decision B (title-suffix centralization) — resolved 2026-07-03

The 2026-06-27 page-meta doc deferred centralizing the `` | ${config.site.title} `` tab/SEO suffix ("Decision B") with explicit instructions to revisit **after v6 cutover** so the work wouldn't be thrown away by the port. Cutover shipped; the revisit happened 2026-07-03 and resolved **in favor of centralizing** (`e970dbc`): pages now pass a bare `title` and `Layout.astro` appends the suffix once (`docTitle`), with `title === config.site.title` guarding the homepage's unsuffixed bare site title. All 10 hand-applied call sites were converted and their dead `config` imports dropped. Verified as a pure refactor — title/og:title/twitter:title/JSON-LD name across every built page diffed byte-identical before/after. This is now a small **deliberate divergence from upstream** (stock v6 hand-applies the suffix per call site): when porting upstream `Layout.astro` changes, preserve the `docTitle` wiring. Recorded in CLAUDE.md's *Page metadata* conventions.

### withBase / locale-readiness reassessment

Originally deferred as a no-op: no `base` path configured, single locale (`en`). Reassessed 2026-07-03 given IGDA's own name — **International** Game Developers Association — the SIG may reasonably want additional-language content at some point, which was not weighed as a real near-term possibility in the original 2026-06-28 audit. This doesn't change the technical facts (it is still a no-op today), but it changes the planning posture from "no-op, ignore" to "no-op today, but worth scoping properly since it's a plausible real need, not a hypothetical one." See backlog #3 for the full scope-of-work and what triggers picking it up.
